...
/*   words and creates a TRies Object from it.       */
/*   It will then use it to TRies the text of        */
...
/*     c++ a3main.matrix.cpp a3.cpp timer.cpp -std=c++0x  */
/*                                                        */
/*   To experiment:                                       */
...
int main(int argc, char* argv[]){
    ...
    createTimer.start();
    TRies checker(wordList,numInitial);
    createTimer.stop();
           ...
           return 0;
            ...
            while(!feof(fp2)&& fscanf(fp2,"%[A-Za-z]*",curr)==1){
                ...
                if(!rc){
                    notfound++;
                    #if DEBUG == ON
                    cout << "the word: " << curr << " should be in the TRies" << endl;
                    #endif
                }
                fscanf(fp2,"%[^A-Za-z]*",curr);
            }
    ...
    if(notfound!=0){
        ...
        if(!(feof(fp2)&&feof(fp3)) || error){
            cout << "your output of words not in TRies does not match expected list" << endl;
